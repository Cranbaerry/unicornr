...
if (!empty($saveOrder))
{
	$saveOrderingUrl = 'index.php?option=com_unicornr_notifications&task=transactionnotifications.saveOrderAjax&tmpl=component';
    HTMLHelper::_('sortablelist.sortable', 'transactionnotificationList', 'adminForm', strtolower($listDirn), $saveOrderingUrl);
}
...
function formatMessages($jsonMessages) {
    ...
    if (json_last_error() === JSON_ERROR_NONE && is_array($messages)) {
        ...
        if (count($messages) === 1) {
            ...
            return "<span title='" . count($messages) . " messages'>" . $firstMessage . 
...
<form action="<?php echo Route::_('index.php?option=com_unicornr_notifications&view=transactionnotifications'); ?>" method="post"
	...
	<div id="j-main-container" class="span10">
		...
		<div id="j-main-container">
			...
			<table class="table table-striped" id="transactionnotificationList">
				<thead>
				<tr>
					<th width="1%" >
					...
					<th class='nowrap'>
						<?php echo JHtml::_('searchtools.sort',  'COM_UNICORNR_NOTIFICATIONS_TRANSACTIONNOTIFICATIONS_DAYS_BEFORE_REMINDER', 'a.days_before_reminder', $listDirn, $listOrder); ?>
					</th>
					<th class='nowrap'>
						<?php echo JHtml::_('searchtools.sort',  'COM_UNICORNR_NOTIFICATIONS_TRANSACTIONNOTIFICATIONS_TIME', 'a.time', $listDirn, $listOrder); ?>
					</th>
					<th class='nowrap'>
						<?php echo JHtml::_('searchtools.sort',  'COM_UNICORNR_NOTIFICATIONS_TRANSACTIONNOTIFICATIONS_MESSAGE', 'a.message', $listDirn, $listOrder); ?>
					</th>
				...
				<?php foreach ($this->items as $i => $item) :
					...
					<tr class="row<?php echo $i % 2; ?>">
						<td class="text-center">
							...
							<td >
							<div class="btn-group">
								<?php echo HTMLHelper::_('jgrid.published', $item->state, $i, 'transactionnotifications.', $canChange, 'cb'); ?>
								<?php if ($canChange) : ?>
									<?php  HTMLHelper::_('actionsdropdown.' . ((int) $item->state === 2 ? 'un' : '') . 'archive', 'cb' . $i, 'transactionnotifications'); ?>
									<?php HTMLHelper::_('actionsdropdown.' . ((int) $item->state === -2 ? 'un' : '') . 'trash', 'cb' . $i, 'transactionnotifications'); ?>
						...
						<td class="">
							<?php if (isset($item->checked_out) && $item->checked_out && ($canEdit || $canChange)) : ?>
								<?php echo JHtml::_('jgrid.checkedout', $i, $item->uEditor, $item->checked_out_time, 'transactionnotifications.', $canCheckin); ?>
							<?php endif; ?>
							<?php if ($canEdit) : ?>
								<a href="<?php echo JRoute::_('index.php?option=com_unicornr_notifications&task=transactionnotification.edit&id='.(int) $item->id); ?>">
									<?php echo $this->escape($item->days_before_reminder); ?>
									</a>
								<?php else : ?>
												<?php echo $this->escape($item->days_before_reminder); ?>
								<?php endif; ?>
