...
/**
 * Methods supporting a list of Unicornr_notifications records.
 *
 * @since  1.6
 */
class Unicornr_notificationsModelStatusupdatenotifications extends \Joomla\CMS\MVC\Model\ListModel
{
	...
	*/
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':' . $this->getState('filter.state');
                ...
                if(!$id || $this->userIDItem($id) || $this->isAdminOrSuperUser() || $this->isUserCreatedItem() || Factory::getUser()->authorise('core.manage', 'com_unicornr_notifications')){
	...
	protected function getListQuery()
	{
		...
		$query->from('`#__unicornr_notifications_transactions_updates` AS a');
		...
		if (!empty($search))
		{
			...
			else
			{
				...
				$query->where('( a.status_type LIKE ' . $search . '  OR  a.message LIKE ' . $search . ' )');
			}
		}
	...
	public function getItems()
	{
		...
		foreach ($items as $oneItem)
		{
			...
			if (isset($oneItem->status_type))
			{
				$values    = explode(',', $oneItem->status_type);
				$textValue = array();
...
.id = $value AND fieldtype = 'status';";
				...
				$oneItem->status_type = !empty($textValue) ? implode(', ', $textValue) : $oneItem->status_type;
			}
		}
